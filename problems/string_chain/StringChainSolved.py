...
def string_chain_solution(lst: List[str]) -> bool:
    ...
    """
    idea:
    we create a graph with 26 nodes representing the alphabet. each string will be represented by a node connecting from
    the node of the first letter to the node of the lase letter.
    then, we need to check if the graph contains an eulerian circuit (closed loop going exactly once through every
    edge). this is true if the next 2 demands are correct:
